...
<body>
	<div class="wrapper">
		...
		<c:choose>
			<c:when test="${staff.role eq 'MANAGER'}">
				<jsp:include page="../sideManager.jsp" />
			</c:when>
			<c:when test="${staff.role eq 'PHARMACIST'}">
		...
		<main class="content-wrapper">
			<div class="profile-container">
				...
				<div class="form-grid">
					...
					<fmt:parseDate var="parsedBirthDate" value="${staff.DOB}" pattern="yyyy-MM-dd" />
					<fmt:formatDate var="birthYear" value="${parsedBirthDate}" pattern="yyyy" />
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    .then(data => {
        if(data) {
            ...
            // Format dynamic date string to standard display dd/MM/yyyy if exists
            if(data.DOB && document.getElementById('DOB')) {
                const dateParts = data.DOB.split("-");
                if(dateParts.length === 3) {
                    document.getElementById('DOB').value = dateParts[2] + "/" + dateParts[1] + "/" + dateParts[0];
                }
            }
            
            // Update Text Nodes within Header Section next to Avatar Circle Layout
    ...
    if (passwordForm) {
        passwordForm.addEventListener('submit', function(e) {
            ...
            // Regex layout for mandatory character requirements
